...
**** google-translate
     ...
     This package allows to translate any word (strings don't seems to
     work AFAIK) using google translate. There are, basically, two
     methods for translating: at-point (which uses "default"
     languages) and smooth (which uses the directions-alist).
     ...
     (when (and (require 'google-translate nil t)
       ...
       (progn
         ;; default languages for translate (when using smooth)
