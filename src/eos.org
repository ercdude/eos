...
*** Environment
    ...
    Attention: if using `user-mail-address`, make sure that it's non
    nil otherwise tramp wont load.
...
*** Beep
    ...
    often. Every time you type Ctrl-G, to stop searching, or stop what
    you were typing and do something else, and so on… you will get the
    bell. To avoid this annoying beep, I prefer to blink the modeline.
...
***** Functions
      ...
      current buffer's file. The eshell is associated with the number
      ARG. You can call C-u 2 M-x eshell-here and the eshell
      associated with the number 2 will be called.

      See (describe-function 'eshell)"
        (interactive "P")
        ;;(when 'not-in-quick-buffer)
        (when 't
          (let ((height (/ (window-total-height) 3)))
            (split-window-vertically (- height))
            (other-window 1))
          ;; substitute this for a generic expression
...
***** Binds
      ...
      (define-key ctl-x-map (kbd "RET") 'eshell-here)
...
***** Pulse
      ...
      PulseAudio is a network-capable sound server program distributed
      via the freedesktop.org project.

      Start Pulse server after emacs initialize. If you are not using
      emacs as the window manager, this is not the place to start it.
...
***** Customize
      ...
      (set-variable 'comint-terminfo-terminal "dumb")
