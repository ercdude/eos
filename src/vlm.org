...
=vlm-= will appear often through this document in reference
to this definition.
...
* GOAL
  ...
  human brain and machine resources, i.e,
  as a =VIRTUAL LISP MACHINE=.

* INIT

  When you start Emacs, it normally attempts to load
  your =init.el file=, that will =load*= and =set= the proper
  values to the customizable library's variables.

  =Org-mode= is used to write and organize the configuration
  ~src/vlm.org~ (this document) in a =literate programming format=,
  meaning it contains =Elisp= source code blocks - that will be tangled
  generating the ~src/init.el~ final script.

  load* : Just load the definitions using the =autoloads= facility
...
***** Customize
      ...
      (customize-set-variable 'lazy-enable-filenotify-flag t)
...
***** Customize
      ...
      (customize-set-variable 'rcirc-server-alist
                              '(("irc.freenode.net"
                                 ...
                                 :encryption tls
                                 :nick "ercdude"
                                 :channels ("#lisp-br #lispgames #freebsd-br"))))
...
***** Binds
      ...
      (define-key vlm-tags-map (kbd "t") 'imenu)
